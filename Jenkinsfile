        ...
        stage('Deploy') {
            steps {
                ...
                sshagent (credentials: ['vagrant']) {
                    ...
                    sh 'ssh vagrant@192.168.33.10 tar -xzvf ${DEPLOYMENT_PATH}/$(ssh vagrant@192.168.33.10 date -d  @${CURRENT_BUILD} +$FORMAT)/themes.tar.gz -C ${DEPLOYMENT_PATH}/$(ssh vagrant@192.168.33.10 date -d  @${CURRENT_BUILD} +$FORMAT) --recursive-unlink'
                    sh 'ssh vagrant@192.168.33.10 tar -xzvf ${DEPLOYMENT_PATH}/$(ssh vagrant@192.168.33.10 date -d  @${CURRENT_BUILD} +$FORMAT)/custom_plugins.tar.gz -C ${DEPLOYMENT_PATH}/$(ssh vagrant@192.168.33.10 date -d  @${CURRENT_BUILD} +$FORMAT) --recursive-unlink'
                    sh 'ssh vagrant@192.168.33.10 tar -xzvf ${DEPLOYMENT_PATH}/$(ssh vagrant@192.168.33.10 date -d  @${CURRENT_BUILD} +$FORMAT)/engine_plugins.tar.gz -C ${DEPLOYMENT_PATH}/$(ssh vagrant@192.168.33.10 date -d  @${CURRENT_BUILD} +$FORMAT) --recursive-unlink'
